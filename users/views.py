...
from users.models import Passport,Adderss
import re
...
def address(request):
    ...
    if request.method == 'GET':
        ...
        # 查询用户的默认地址
        addr = Adderss.objects.get_default_address(passport_id=passport_id)
        return render(request, 'users/user_center_site.html', {'addr': addr, 'page': 'adderss'})
    else:
        # 添加收货地址
        # 1.接收数据
        ...
        # 3.添加收货地址
        Adderss.objects.add_one_address(passport_id=passport_id,
